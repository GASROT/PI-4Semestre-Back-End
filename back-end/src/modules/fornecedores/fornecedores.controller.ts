...
import type { CreateFornecedorDto, FornecedorParamsDto, UpdateFornecedorDto } from './fornecedores.dto';

export const fornecedoresController = {
  async create(req: Request, res: Response) {
    const fornecedor = await fornecedoresService.create(req.body as CreateFornecedorDto);
  ...
  async getById(req: Request, res: Response) {
    const { id } = req.params as FornecedorParamsDto;
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params as FornecedorParamsDto;
    const fornecedor = await fornecedoresService.update(id, req.body as UpdateFornecedorDto);
    res.status(200).json({ success: true, data: fornecedor });
  },
