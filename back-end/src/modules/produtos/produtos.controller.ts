...
import type { CreateProdutoDto, ProdutoParamsDto, UpdateProdutoDto } from './produtos.dto';

export const produtosController = {
  async create(req: Request, res: Response) {
    const produto = await produtosService.create(req.body as CreateProdutoDto);
  ...
  async getById(req: Request, res: Response) {
    const { id } = req.params as ProdutoParamsDto;
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params as ProdutoParamsDto;
    const produto = await produtosService.update(id, req.body as UpdateProdutoDto);
    res.status(200).json({ success: true, data: produto });
  },
